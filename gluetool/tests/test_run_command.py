...
def _assert_logging(log, record_count, cmd, stdout=None, stderr=None, stdout_index=2, stderr_index=3):
    # pylint: disable=too-many-arguments

    assert len(log.records) == record_count
    # assert all([r.levelno == logging.DEBUG for r in records])

    assert log.records[0].message == 'run command: {}'.format(cmd)
...
def test_sanity(popen, log):
    ...
    popen.assert_called_once_with(['/bin/ls', '/'], stdout=subprocess.PIPE, stderr=subprocess.PIPE)

    _assert_logging(log, 4, "cmd='['/bin/ls', '/']', kwargs={'stderr': 'PIPE', 'stdout': 'PIPE'}",
                    stdout='stdout:\n---v---v---v---v---v---\nroot listing\n---^---^---^---^---^---',
                    stderr='stderr:\n---v---v---v---v---v---\n\n---^---^---^---^---^---')
...
def test_oserror(popen, log, actual_errno, expected_exc, expected_message):
    ...
    _assert_logging(log, 1, "cmd='['/bin/foo']', kwargs={'stderr': 'PIPE', 'stdout': 'PIPE'}")
...
def test_exit_code_error(popen, log):
    ...
    _assert_logging(log, 4, "cmd='['/bin/foo']', kwargs={'stderr': 'PIPE', 'stdout': 'PIPE'}",
...
def test_forwarding(popen, log, actual_comm, stdout, stderr):
    ...
    output = run_command(['/bin/foo'], stdout=stdout[0], stderr=stderr[0])
...
def test_invalid_stdout(popen, log):
    ...
    with pytest.raises(AttributeError, match=r"^'tuple' object has no attribute 'fileno'$"):
        run_command(['/bin/foo'], stdout=(13, 17))

    _assert_logging(log, 1, "cmd='['/bin/foo']', kwargs={'stderr': 'PIPE', 'stdout': '(13, 17)'}")
